...
function Perfil() {
  ...
  const guardar = async (dataForm) => {
    const url = `${import.meta.env.VITE_BACKEND_URL}/usuarios/perfil/actualizar/${usuario._id}`;

    // 🛠️ FIX: el input de email está disabled, así que react-hook-form
    // no lo incluye en dataForm. El backend exige "email" en el body,
    // así que lo agregamos a mano desde el estado actual del usuario.
    const payload = {
      ...dataForm,
      email: usuario.email,
    };

    const response = await fetchDataBackend(url, payload, "PUT", {
      Authorization: `Bearer ${token}`,
    });
          ...
          >
            ...
            <div>
              ...
              <input
                type="email"
                disabled={true}
                className="w-full border rounded-xl px-4 py-3 text-sm text-gray-600 outline-none border-pink-100 bg-[#fdf6f9]"
                {...register("email")}
              />
            </div>
            <div>
